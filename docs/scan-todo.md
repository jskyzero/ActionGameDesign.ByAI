...
本轮按用户勾选的 7 项与另列的 13 项整理，共 20 项：17 项已写成笔记，3 项资料不足保留为待施工。此前 53 篇及本轮完成项统一见[已收录归档](scan-archive.md)。归档不代表本轮重新核实了历史正文。收录范围遵循 [EXTRACTION.md](EXTRACTION.md)。
...
这些条目已建立 `wip` 文件，不计入完成归档。详情页记录核实结果与缺口；取得材料后继续整理，不重复建文。

- [《怪物猎人：荒野》：输入与动作状态如何进入共用系统](../src/content/docs/capcom/2025-monster-hunter-wilds-ace.mdx)：官方讲者说明以同时按键的时间容差、大量动作的状态转换为例，解释 ace 共用系统要面对的需求。本次未取得足以核实具体机制的完整讲义或录像，不能据概要声称其使用了某种输入缓存时长或状态机方案。
- [《Kick-Flight》：单指操控与自动镜头如何支撑空中对战](../src/content/docs/other/2020-kick-flight-controls-camera.mdx)：已核实 CEDEC 2020 的讲题、讲者与 CEDiL 入口。官方概要明确涉及操作失误辅助、自动动态镜头和三维方向提示；资料页列出 MP4，获取需登录，本次未取得录像正文。
- [《战神 III》：把泰坦做成可玩的活体关卡](../src/content/docs/santa-monica/2012-gow3-titans.mdx)：讲题与年份已由 GDC Vault 和官方录像说明交叉确认。YouTube 的 2018 年是上传时间，演讲举办于 2012 年。尚缺可核读的完整讲义或字幕，待补移动表面、角色接触与关卡交互的具体案例。
