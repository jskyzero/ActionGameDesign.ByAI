...
1. **信息密度分层**（详见 [`EXTRACTION.md`](EXTRACTION.md)）：
   ...
   - 正文 Details 挂在 `##` 小节下，按问题、机制、案例与取舍组织；段落简短，但不限制为三个观点。保留失败原型、实现条件及可定位的来源。
...
4. 只有资料充分且正文完成才标 `status: done`；资料不足的主题写入 `docs/scan-todo.md`，不生成占位文章。
...
7. 先核读完整可得材料，图表需查看原图。每个主要观点用具体案例支撑，区分作者事实和本文推论。配图注明来源与页码，文字能独立解释图意。详见 `EXTRACTION.md` 第 6 节。
